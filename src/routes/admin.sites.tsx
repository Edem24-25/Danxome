...
function SitesAdmin() {
  ...
  return (
    <DashboardShell
      ...
      crumbs={[{ label: "Administration", to: "/admin" }, { label: "Sites" }]}
      actions={
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold text-forest-deep">{sites.length}</p>
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Avec visite virtuelle</p>
                <p className="text-2xl font-semibold text-forest-deep">
                  {sites.filter((s) => s.virtuel).length}
                </p>
              </div>
            </div>
          </div>
          <div className="rounded-lg border border-border bg-card p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-semibold text-forest-deep">
                  {sites.length > 0
                    ? (sites.reduce((sum, s) => sum + (s.note ?? 0), 0) / sites.length).toFixed(1)
                    : "—"}
                </p>
              </div>
            </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {s.region} · {s.type} · {s.prix > 0 ? `${s.prix} FCFA` : "Gratuit"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1 text-sm">
                      ...
                      <span className="text-xs text-muted-foreground">({s.avis_count})</span>
                    </div>
                    <Button asChild variant="outline" size="sm">
                      <Link to="/tourisme/sites/$slug" params={{ slug: s.slug }}>
                        Voir
                      </Link>
                    </Button>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem
                ...
                : "Remplissez les champs pour créer un site touristique."}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4 sm:grid-cols-2">
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1">
                ...
                <Input
                  ...
                  step="0.01"
                  value={form.coords_x || ""}
                  onChange={(e) => setForm({ ...form, coords_x: Number(e.target.value) })}
                />
              </div>
              <div className="flex-1">
                ...
                <Input
                  ...
                  step="0.01"
                  value={form.coords_y || ""}
                  onChange={(e) => setForm({ ...form, coords_y: Number(e.target.value) })}
                />
              </div>
            </div>
            <div className="sm:col-span-2">
            ...
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                id="virtuel"
                checked={form.virtuel}
                onChange={(e) => setForm({ ...form, virtuel: e.target.checked })}
